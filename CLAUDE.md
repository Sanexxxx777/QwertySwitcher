...
## Лицензирование (v0.4.0, 03.08.2026) — ⛔СНЯТО в 0.10.0 (08.09.2026), ниже история для сервера S1
⚠️Клиентского кода лицензии в приложении больше нет (см. секцию v0.10.0); сервер `S1:/root/qsw-license/` остановлен 08.09 (pm2 stopped, код на месте). Абзац ниже — история.
...
- ⛔**Сервер лицензий на S1 ОСТАНОВЛЕН 08.09 по команде Саши** (`pm2 stop qsw-license` + `pm2 save`, порт 127.0.0.1:8377 закрыт, cron нет; код `/root/qsw-license/` оставлен, nginx-location `/qsw/` в developer-contact-api висит и отдаёт 502 — снять при следующем касании nginx). Следствие: старые сборки 0.8.x–0.9.x у Дани и на мамином Маке через 3 дня офлайн-окна перестанут корректировать — им нужен DMG 0.10.0 с витрины (Дане прислать, маму Саша обновляет сам). Откат: `pm2 start qsw-license`.
...
- ✅~~оплата / офлайн-триал / якорь первого запуска / Robokassa~~ — контур снят целиком в 0.10.0 (приложение бесплатное); сервер S1 остановлен 08.09. **low** — снять nginx-location `/qsw/` из конфига developer-contact-api на S1 при следующем касании nginx (сейчас 502, не мешает).
