...
### TODO
- 🎯**high (принцип, 16.08.2026, от владельца)** — «русский коряво написан ⇒ я пишу на английском; программа должна это понимать». Мусорность ТЕКУЩЕГО прочтения обязана быть самостоятельным сигналом, симметрично в обе стороны. Сегодня архитектура однобокая («победитель обязан быть в словаре»), отсюда весь класс «не сработало» на словах вне словаря (qwerty, баги, имена, жаргон). План: метрика мусорности на `NGramAnalyzer` (forbidden-биграммы, отсутствие гласных, невозможные стечения) → правило «текущее мусор И целевое не мусор ⇒ конвертировать даже без словарного попадания» → гейт против путей/флагов/аббревиатур → **обязательная корпусная верификация** (`scratchpad/false_switch_sim.py`, топ-10000×2: ложные не выросли, пропуски упали). Ослаблять словарный ров без чисел НЕЛЬЗЯ.
...
- **medium** — чистка словарей: 650 мусорных пар в `en_US.txt`, 774 в `ru_RU.txt`, обрубки в ru («прив», «сдел», «рабо»). Код от них защищён закрытыми списками на длине 2, но чистка снимает класс целиком и разблокирует `minLength` 4→3 у мгновенной коррекции.
- **medium** — 5×`slow tap callback` 18-22мс подряд (13.08): macOS может отключить перехват при повторных таймаутах, не расследовано.
