...
### TODO
- **high** — 13.08.2026, в рабочем дереве (не закоммичено, не установлено): AX-проба замены больше не копирует значение поля (`lengthAndCaret` через `kAXNumberOfCharacters`, фолбэк на `valueAndCaret`); пороги Double Shift выровнены на 1 символ в run-пути и в истории. 397 passed. Полевой verify: строка `ax probe: no char-count attribute` в логе = Ghostty не отдаёт счётчик и выигрыша нет; «b»+пробел+DS и одиночная «.» вместо «/» обязаны конвертироваться.
