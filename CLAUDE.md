...
⚠️Клиентского кода лицензии в приложении больше нет (см. секцию v0.10.0). Абзац оставлен как карта серверной части `S1:/root/qsw-license/`, которую пока держим для старых сборок.
...
- ⚠️**Сервер лицензий на S1 (`qsw-license`, pm2) НЕ выключен сознательно:** у Дани и на мамином Маке стоят 0.8.x–0.9.x, которые ходят на `/v1/hello`; без сервера через 3 дня офлайн-окна они перестанут корректировать. Снимать только после того, как обе машины обновятся на 0.10.0 (тогда — `pm2 stop qsw-license`, nginx-location `/qsw/` и `store/key.html` уже удалён с витрины).
...
### TODO
- ✅~~оплата / офлайн-триал / якорь первого запуска / Robokassa~~ — контур снят целиком в 0.10.0 (приложение бесплатное); сервер S1 ждёт обновления старых сборок у Дани и мамы, потом `pm2 stop qsw-license`.
