...
- BloomFilter + NSSpellChecker (словарь 714K слов; Bloom ~1MB + префикс-индекс мгновенной коррекции ~11MB RAM, строится асинхронно). 🔴С 0.9.2 Bloom — только префильтр: попадание подтверждается точным бинпоиском по тому же отсортированному индексу (`WordDictionary.isConfirmedWord`), пока индекс строится — Bloom-only.
...
- Публикация на `~/Projects/web/store` (карточка Qwerty Switcher, новый DMG-линк) — **по прямой команде Саши**, не самоуправство Codex (уточнено 21.08 вечером — моё первое чтение диффа было неверным).
...
Коммит `9c7239d` (Info.plist 0.7.0→0.7.1). Витрина обновлена и запушена (коммит `550de23` в `Sanexxxx777/store`, по прямой команде Саши).
...
Источник — жалоба Саши «после 0.11.2 стало хуже» + лог 19.09 16:57 → 20.09 18:17.
...
- **Открыто, не трогали:** learned `en:hub` (count=3) на границе перебивает настоящее «руб» — в `detect()` learned-хит даёт 80+ и у него нет own-word-гарда, какой есть на instant-пути (`own.wordLevel == 0`). Решение Саши.
...
- 🔴**Коды клавиш вернулись в подробный лог** (решение Саши 19.09). 0.11.1 убрал их ради приватности и заодно убил word-level анализ: по логу без `key kc=` нельзя ответить, на настоящем слове сработала правка или на мусоре, а это единственный вопрос, ради которого лог существует. Producer в `KeyboardMonitor` + сток-гейт в `DebugLog.log` сняты; защита осталась той, что и была: лог выключен по умолчанию, файл 0600 в каталоге 0700, а «Собрать отчёт» эти строки вырезает (`DiagnosticsExportService.filterReportLog`). Текст приватности в `PrivacyService` и подпись тумблера в «Логи» переписаны честно — приложение не должно врать о себе.
- **«нфт» → «yan» — ложная правка, подтверждена Сашей** (дважды 18.09 в Ghostty). Причина не в остовe: слово без гласной, на русской стороне 0 очков, а `yan` есть в `en_US.txt` (355k слов, скрабблный список). Фикс — `нфт` в `ru_RU.txt`; регрессия `OwnerAbbreviationRegressionTests` держит обе стороны: «нфт» не правится, «ьщвуд» по-прежнему становится «model».
- **Не сделано намеренно:** чистка `en_US.txt` от подобных трёхбуквенных редкостей (системная работа, отдельная задача), короткие токены (`learned: fired len=2|3` в поле так и не наблюдался — Саша бьёт DS ДО пробела, авто-починке по границе шанса не было).
...
- ⛔**Сервер лицензий ОСТАНОВЛЕН 08.09 по команде Саши** (процесс остановлен, порт закрыт, код сохранён; оставшаяся nginx-локация отдаёт 502 — снять при следующем касании конфига; порядок команд и откат — в памяти `project_qwerty_payments_next_session`). Следствие: старые сборки 0.8.x–0.9.x у Дани и на мамином Маке через 3 дня офлайн-окна перестанут корректировать — им нужен DMG 0.10.0 с витрины (Дане прислать, маму Саша обновляет сам). Откат: `pm2 start qsw-license`.
...
Коммиты `47018ea`(волна 1: модули) → `75e1c26`(волна 2: интеграция) → `5b2e99a`(лог) → `0109cab`(волна 3: UI) → `b8450f6`(бамп). Дизайн — ТОЛЬКО по спеке `~/.claude/plans/qwerty-learning-spec-20260824.md` (22 правки адверсариального ревью уже внесены — не переизобретать по памяти). Витрина обновлена и запушена (по прямой команде Саши).
...
- **high** — полевой verify 0.8.0 (обучение): дождаться в debug.log цепочку `learned: recorded` → `learned: promoted` → `learned: fired path=…` на реальных DS-починках Саши; после первых fired — снять долю срабатываний и ложных (revert → exception). На 25.08 15:50 поле ещё не началось (0.8.0 только установлена).
...
- ✅~~in-app updater отсутствует~~ — есть с 0.11.0 (opt-in). **low** — нотаризация (Stage 2) отложена решением Саши 10.09 (смена identity сбросит TCC у всех текущих установок; делать, когда автоапдейт уже стоит у людей и сможет провести миграцию).
