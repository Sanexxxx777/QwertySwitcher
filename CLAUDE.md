...
- 🔴**Codex сам запушил на публичный `~/Projects/web/store`** (карточка Qwerty Switcher, новый DMG-линк) — уже на GitHub Pages. Нарушает правило «публикацию жму сама» (Саша не подтверждал явно). Оставлено как есть по факту ревью 21.08 — контент точный, откат не требовался.
- Мёртвый код подчищен при ревью: `ExceptionsService.isCurrentAppExcepted()`/`isInstantCorrectionBlockedForCurrentApp()` удалены (0 вызовов в проде — заменены прямым чтением `appProfile` из кэша).
