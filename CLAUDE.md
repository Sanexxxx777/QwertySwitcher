...
### TODO
- **high** — оплата не включена: сервер лицензий готов (заказы + вебхук + выдача ключа только после оплаты, 21 pytest), но `/v1/buy` отдаёт 503 без `CRYPTOBOT_TOKEN`. Ждём токен и доступы Robokassa → чеклист в памяти `project_qwerty_payments_next_session.md`. **Сквозная покупка ни разу не проходила боем** — вебхук проверен только тестами.
- **high** — фикс офлайн-триала (якорь в Keychain + окно 3 дня вместо 14, коммит `7a2eaba`) собран и протестирован, но НЕ выпущен: версия 0.8.0 не бампнута, DMG не собран, на витрине старая сборка. Пользователи с 0.8.0 всё ещё могут крутить бесконечные офлайн-триалы.
- **medium** — новая логика якоря первого запуска (`firstSeenCandidates`, Keychain) не покрыта тестом: изолировать без реального Keychain не получается. Проверена ревью.
- **low** — Robokassa ResultURL пойдёт через общий `limit_req` nginx (зона `developer_contact_rate`, burst=10) вместе с вебхуком CryptoBot; при всплеске доставок возможен 429 — вынести в отдельный location, если поймаем.
...
- **low** — in-app updater отсутствует; нотаризация (Stage 2). (DMG на витрине синхронен — 0.8.0, 25.08.)
