...
## Post-0.11.3 (23.09.2026, not released) — smart case stops inventing capitals

Source: owner's report «стал делать много ошибок» + verbose log 21.09 20:47 → 23.09 12:15. The switcher's own decisions were clean (56 automatic corrections, all right; 0 missed wrong-layout dictionary words; 0 slow tap callbacks, 0 tap disables); most of the garbled text was the owner's own keystroke typos (adjacent-letter swaps) that passed through untouched. Real defect: smart case capitalized wrongly 4 times of 22.
- `SentenceStartTracker` needs a gap now: `.!?` arms it, an empty `proseBoundary` (Space/Enter/Tab) confirms it, so a word glued to the period stays lowercase («узна.т» — RU "." hit instead of "ю"; «т.е»). Digits among `pendingLeadingSymbols` (`pendingLeadHasDigit`) veto/reset: the sentence starts with the number. Backspace with an empty current word resets unless the pending leading symbols are digit-free (a deleted second "." must keep the capital).
- Replay over the 22 field events: 18 still fire, exactly the 4 wrong ones go silent. Tests 954/0/10 + contracts. Field examples in this repo are synthetic — never paste reconstructed owner text here.
- ⚠️Log timestamps are WRITE time (`DebugLog` async utility queue): inter-key dt read off the log is not evidence, line order is. Replayed queued keys re-enter the session tap and show up in the `key kc=` trace.
- Open: `finishReplacement` sets `isPaused=false` before draining the queue — a live key already waiting in the tap port could overtake the replay (sub-ms window, not observed). `applyYoficator` logs nothing on success (14 silent replacements in 39 h, invisible in the log).
