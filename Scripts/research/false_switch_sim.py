...
# ============================================================================
# Golden decisions (plan 009, Step 5) — shared ground truth between this port
# and Sources/QwertySwitcher/Tests/PortParityTests.swift's
# `runGoldenDecisions()`. Vectors were generated by running the real Swift
# detector once (a throwaway harness, deleted after use), never hand-derived.
# ============================================================================
def check_golden(path):
    """--check-golden <path>: replay every vector through detect_boundary and
    exit non-zero on any mismatch, printing each one. A vector's "typed" is
    always the Latin/QWERTY physical-key identity (Core/LanguageDetector.swift
    test-fixture convention), so it is translated into detect_boundary's
    own_word (the OWN-layout rendering) here — own_word=typed when layout is
    "en" (Latin keys already render as themselves), own_word=translit(typed,
    KEY2RU) when layout is "ru" (physical keys render as Cyrillic). A vector's
    absolute "context" ("en"/"ru"/"none") is mapped to detect_boundary's
    relative context parameter ("same"/"target"/"none") from `layout`.
    """
    with open(path, encoding="utf-8") as f:
        vectors = json.load(f)
    ...
    mismatches = []
    for v in vectors:
        typed, layout, context, expect = v["typed"], v["layout"], v["context"], v["expect"]
        own_word = typed if layout == "en" else translit(typed, KEY2RU)
        if own_word is None:
            mismatches.append((v, "own_word translit failed (a character outside KEY2RU)"))
            continue
        if context == "none":
            ctx_param = "none"
        elif context == layout:
            ...
            ctx_param = "target"
        result, winner_lang, _winner_core, _other_reading = detect_boundary(own_word, layout, context=ctx_param)
        actual = f"switchTo:{winner_lang}" if result == "switchTo" else "noSwitch"
        if actual != expect:
            mismatches.append((v, actual))

    if mismatches:
        print(f"[check-golden] {len(mismatches)}/{len(vectors)} MISMATCH:")
        for v, actual in mismatches:
            print(f"  {v['typed']!r} [{v['layout']}, ctx={v['context']}]: expected {v['expect']}, got {actual}")
        return 1
    print(f"[check-golden] {len(vectors)}/{len(vectors)} vectors agree with the Swift detector")
    return 0
...
if __name__ == "__main__":
    if "--check-golden" in sys.argv:
        _idx = sys.argv.index("--check-golden")
        if _idx + 1 >= len(sys.argv):
            print("usage: false_switch_sim.py --check-golden <path>")
            sys.exit(2)
        sys.exit(check_golden(sys.argv[_idx + 1]))
